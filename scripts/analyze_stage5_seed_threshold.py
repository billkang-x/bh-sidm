"""Analyze the refined seed-mass threshold for influence-gated capture."""
...
MANIFEST = ROOT / "hpc" / "stage5_seed_threshold.tsv"
RESULTS = ROOT / "results" / "stage5" / "seed_threshold"
SUMMARY = ROOT / "results" / "stage5" / "seed_threshold_summary.csv"
STATISTICS = ROOT / "results" / "stage5" / "seed_threshold_statistics.json"
FIGURE = ROOT / "results" / "stage5" / "figures" / "stage5_seed_threshold.png"
...
def main() -> None:
        ...
        cases.append(
            {
                ...
                "flux_capture_mass_threshold_msun": float(metadata["dark_flux_capture_mass_threshold_msun"]),
    ...
    for seed in sorted({case["black_hole_seed_msun"] for case in cases}):
        ...
        reaches = [np.isfinite(case["time_to_1e7_msun_myr"]) for case in pair]
        classification = "ambiguous"
        if all(reaches):
            classification = "robust_success"
            robust_success.append(seed)
        elif not any(reaches):
        ...
        by_seed[f"{seed:.0f}"] = {
            ...
            "small_boundary_final_mass_msun": pair[0]["final_black_hole_mass_msun"],
            "large_boundary_final_mass_msun": pair[1]["final_black_hole_mass_msun"],
            "boundary_relative_difference": relative_difference(
                pair[0]["final_black_hole_mass_msun"],
                pair[1]["final_black_hole_mass_msun"],
            ),
            "small_boundary_time_to_1e7_myr": (
                pair[0]["time_to_1e7_msun_myr"]
                if reaches[0]
                else None
            ),
            "large_boundary_time_to_1e7_myr": (
                pair[1]["time_to_1e7_msun_myr"]
                if reaches[1]
                else None
            ),
        }
    ...
    for boundary, marker in zip((0.05208333333333334, 0.10416666666666667), ("o", "s")):
        selected = sorted(
            (case for case in cases if np.isclose(case["reference_r_min_over_influence"], boundary)),
    ...
    for seed in sorted({case["black_hole_seed_msun"] for case in cases}):
        time, mass = histories[(seed, 0.05208333333333334)]
        axes[1].plot(time, mass, label=f"{seed:.0e}")
